...
class RoutesTest < ActionDispatch::IntegrationTest
  def test_route_to_sitemap
    assert_routing({:method => "get", :path => "/sitemap.xml"}, {:controller => "sitemap", :action => "index", :format => "xml"})
  end
  ...
  def test_route_to_nodes
    ...
    assert_recognizes({:controller => "nodes", :action => "destroy", :id => "1"}, {:method => "delete", :path => "/b/1"})
    assert_routing({:method => "get",    :path => "/b/1/delete"}, {:controller => "nodes", :action => "delete",  :id => "1"})
    assert_routing({:method => "delete", :path => "/b/1/delete"}, {:controller => "nodes", :action => "destroy", :id => "1"})
  end
  ...
  def test_route_to_notes
    ...
    assert_recognizes({:controller => "notes", :action => "destroy", :id => "1"}, {:method => "delete", :path => "/l/1"})
    assert_routing({:method => "get",    :path => "/l/1/delete"}, {:controller => "notes", :action => "delete",  :id => "1"})
    assert_routing({:method => "delete", :path => "/l/1/delete"}, {:controller => "notes", :action => "destroy", :id => "1"})
  end
end
